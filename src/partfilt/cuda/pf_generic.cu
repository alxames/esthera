...
__device__ __forceinline__ void sampling(state* const particle_data, const control control_data, const float* const d_random, const float dt);
	...
	const float dt)
{
	...
	sampling(&d_particle_data[idx], control_data, &d_random[NUM_STATE_VARIABLES*idx], dt);

#ifdef TIMING_DEVICE
	const clock_t s1 = clock();
#endif /* TIMING_DEVICE */

	const float weight = importance_weight(&d_particle_data[idx], measurement_data);
...
__global__ void resampling(
	state* const d_particle_data,
	const state* const d_particle_data_tmp,
...
*/
	...
	/* particles */
	state* d_particle_data;
	CUDA_CALL(cudaMalloc(&d_particle_data,
		sizeof(state) * num_particles * num_blocks));

	state* d_particle_data_tmp;
	CUDA_CALL(cudaMalloc(&d_particle_data_tmp,
		sizeof(state) * num_particles * num_blocks));
...
*/
		...
		if (num_transfer > 0)
		{
			exchange_ring<<< num_blocks, num_transfer>>>(d_particle_data_tmp, d_particle_weights, num_particles);
...
#endif /* MTGP */
	CUDA_CALL(cudaFree(d_particle_data));
	CUDA_CALL(cudaFree(d_particle_data_tmp));
