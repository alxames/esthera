...
//			float  local_angles2;
			CUDA_CALL(cudaMemcpy(&local_angles1, &d_particle_data_tmp.angles1[global_winner_index*num_particles], sizeof(float4), cudaMemcpyDeviceToHost));
...
//			lpw.angles[4] = local_angles2;
			...
			float4 local_pos;
			CUDA_CALL(cudaMemcpy(&local_pos, &d_particle_data_tmp.pos[global_winner_index*num_particles], sizeof(float4), cudaMemcpyDeviceToHost));
			lpw.y  = local_pos.x;
			lpw.z  = local_pos.y;
			lpw.vY = 0;
			lpw.vZ = 0;

			CUDA_CALL(cudaMemcpy(&lpww, &d_particle_weights[global_winner_index*num_particles], sizeof(float), cudaMemcpyDeviceToHost));
