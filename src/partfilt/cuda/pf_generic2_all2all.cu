...
#include <pf_model_soa_packed_code.cuh>
...
__global__ void block_sort(
	...
	float* d_block_winners_weight,
	int* d_block_winners_index)
{
	...
	if(tid < num_transfer)
	{
		d_block_winners_weight[blockIdx.x*num_transfer + tid] = shared_weights[tid];
		d_block_winners_index [blockIdx.x*num_transfer + tid] = blockshift + shared_index[tid];
	}
	...
	const int dimy)
{
	...
	//LEFT
	particle_set(d_particle_data, (left*num_particles) + num_particles - tid - 1, d_particle_data, (bid*num_particles)+tid, num_particles, num_blocks);
	d_particle_weights           [(left*num_particles) + num_particles - tid - 1] = w;

	//RIGHT
	particle_set(d_particle_data, (right*num_particles) + num_particles - tid - 1 + num_transfer, d_particle_data, (bid*num_particles)+tid, num_particles, num_blocks);
	d_particle_weights           [(right*num_particles) + num_particles - tid - 1 + num_transfer] = w;

	//TOP
	particle_set(d_particle_data, (top*num_particles) + num_particles - tid - 1 + (2*num_transfer), d_particle_data, (bid*num_particles)+tid, num_particles, num_blocks);
	d_particle_weights           [(top*num_particles) + num_particles - tid - 1 + (2*num_transfer)] = w;

	//BOTTOM
	particle_set(d_particle_data, (bottom*num_particles) + num_particles - tid - 1 + (3*num_transfer), d_particle_data, (bid*num_particles)+tid, num_particles, num_blocks);
	...
	int* d_block_winners_index)
{
	...
	const int target_index = d_block_winners_index[tid];

	particle_set(d_particle_data, (bid*num_particles) + num_particles - tid - 1, d_particle_data,     target_index, num_particles, num_blocks);
	d_particle_weights           [(bid*num_particles) + num_particles - tid - 1] = d_particle_weights[target_index];
}
...
int main(int argc, char* argv[])
{
	...
	while (-1 != (opt = getopt(argc, argv, "m:N:t:r:l:d:"))) {
		switch (opt)
		{
			...
			case 'd':
				...
				break;
			default: /* '?' */
				fprintf(stderr, "Usage: %s [-m #particles] [-N #cores] [-t #transfer] [-r resampling_frequency] [-l loop_count] [-d device] input_file\n", argv[0]);
				exit(EXIT_FAILURE);
		}
	}
	...
	if ((2*num_transfer) > num_particles)
		continue;
...
*/
	...
	{
		CUDA_CALL(cudaMalloc(&d_particle_data.angles1, sizeof(float4) * num_particles * num_blocks));
//		CUDA_CALL(cudaMalloc(&d_particle_data.angles2, sizeof(float)  * num_particles * num_blocks));
		CUDA_CALL(cudaMalloc(&d_particle_data.pos,     sizeof(float4) * num_particles * num_blocks));

		state_data init_particle_data;// = (state_data*)malloc(sizeof(state_data));

		init_particle_data.angles1 = (float4*)malloc(sizeof(float4) * num_particles * num_blocks);
//		init_particle_data.angles2 = (float*) malloc(sizeof(float)  * num_particles * num_blocks);
		init_particle_data.pos     = (float4*)malloc(sizeof(float4) * num_particles * num_blocks);
		...
		for (int i=0; i < (num_blocks*num_particles); ++i)
		{
			...
			{
				if      (j==0){init_particle_data.angles1[i].x = 0;}
				else if (j==1){init_particle_data.angles1[i].y = 0;}
				else if (j==2){init_particle_data.angles1[i].z = M_PI_2;}
				else if (j==3){init_particle_data.angles1[i].w = 0;}
//				else if (j==4){init_particle_data.angles2[i]   = a;}
			}
			init_particle_data.pos[i].x = 0;
			init_particle_data.pos[i].y = 0.315;
			init_particle_data.pos[i].z = 0;
			init_particle_data.pos[i].w = 0;
		}
		CUDA_CALL(cudaMemcpy(d_particle_data.angles1, init_particle_data.angles1, sizeof(float4)*num_particles*num_blocks, cudaMemcpyHostToDevice));
//		CUDA_CALL(cudaMemcpy(d_particle_data.angles2, init_particle_data.angles2, sizeof(float) *num_particles*num_blocks, cudaMemcpyHostToDevice));
		CUDA_CALL(cudaMemcpy(d_particle_data.pos,     init_particle_data.pos,     sizeof(float4)*num_particles*num_blocks, cudaMemcpyHostToDevice));
...
//		free(init_particle_data.angles2);
	...
	{
		CUDA_CALL(cudaMalloc(&d_particle_data_tmp.angles1, sizeof(float4) * num_particles * num_blocks));
//		CUDA_CALL(cudaMalloc(&d_particle_data_tmp.angles2, sizeof(float)  * num_particles * num_blocks));
		CUDA_CALL(cudaMalloc(&d_particle_data_tmp.pos,     sizeof(float4) * num_particles * num_blocks));
	}
	...
	thrust::device_vector<float> d_block_winners_weight(num_blocks*num_transfer);
	thrust::device_vector<int>   d_block_winners_index (num_blocks*num_transfer);
...
//	control control_data;
	...
	local_particle_data.angles1 = (float4*)malloc(sizeof(float4)*num_blocks*num_particles);
//	local_particle_data.angles2 = (float*) malloc(sizeof(float) *num_blocks*num_particles);
	local_particle_data.pos     = (float4*)malloc(sizeof(float4)*num_blocks*num_particles);
...
//			control_data,
		...
		/*CUDA_CALL(cudaMemcpy(
			...
			sizeof(state)*num_blocks*num_particles,
			cudaMemcpyDeviceToHost));*/
		CUDA_CALL(cudaMemcpy(local_particle_data.angles1, d_particle_data.angles1, sizeof(float4)*num_blocks*num_particles, cudaMemcpyDeviceToHost));
//		CUDA_CALL(cudaMemcpy(local_particle_data.angles2, d_particle_data.angles2, sizeof(float) *num_blocks*num_particles, cudaMemcpyDeviceToHost));
		CUDA_CALL(cudaMemcpy(local_particle_data.pos,     d_particle_data.pos,     sizeof(float4)*num_blocks*num_particles, cudaMemcpyDeviceToHost));
		CUDA_CALL(cudaMemcpy(local_particle_weights,      d_particle_weights,      sizeof(float) *num_blocks*num_particles, cudaMemcpyDeviceToHost));
		...
		block_sort<<< num_blocks, num_particles/2>>>(
				...
				num_particles,
				num_transfer,
				thrust::raw_pointer_cast(&d_block_winners_weight[0]),
				thrust::raw_pointer_cast(&d_block_winners_index[0]));
		e = cudaGetLastError();
		if (e != cudaSuccess) {
		...
		/*CUDA_CALL(cudaMemcpy(
			...
			sizeof(state)*num_blocks*num_particles,
			cudaMemcpyDeviceToHost));*/
		CUDA_CALL(cudaMemcpy(local_particle_data.angles1, d_particle_data_tmp.angles1, sizeof(float4)*num_blocks*num_particles, cudaMemcpyDeviceToHost));
//		CUDA_CALL(cudaMemcpy(local_particle_data.angles2, d_particle_data_tmp.angles2, sizeof(float) *num_blocks*num_particles, cudaMemcpyDeviceToHost));
		CUDA_CALL(cudaMemcpy(local_particle_data.pos,     d_particle_data_tmp.pos,     sizeof(float4)*num_blocks*num_particles, cudaMemcpyDeviceToHost));
		CUDA_CALL(cudaMemcpy(local_particle_weights,      d_particle_weights,          sizeof(float) *num_blocks*num_particles, cudaMemcpyDeviceToHost));
		...
		thrust::sort_by_key(d_block_winners_weight.begin(), d_block_winners_weight.end(), d_block_winners_index.begin(), thrust::greater<float>());
		int global_winner_index = d_block_winners_index[0];
...
*/
		...
		//for (int i=0; i < NUM_ANGLES; ++i)
		{
			...
			float4 local_angles1;
//			float  local_angles2;
			CUDA_CALL(cudaMemcpy(&local_angles1, &d_particle_data_tmp.angles1[global_winner_index], sizeof(float4), cudaMemcpyDeviceToHost));
//			CUDA_CALL(cudaMemcpy(&local_angles2, &d_particle_data_tmp.angles2[global_winner_index*num_particles], sizeof(float),  cudaMemcpyDeviceToHost));
			lpw.angles[0] = local_angles1.x;
			lpw.angles[1] = local_angles1.y;
			lpw.angles[2] = local_angles1.z;
			lpw.angles[3] = local_angles1.w;
//			lpw.angles[4] = local_angles2;

			float4 local_pos;
			CUDA_CALL(cudaMemcpy(&local_pos, &d_particle_data_tmp.pos[global_winner_index], sizeof(float4), cudaMemcpyDeviceToHost));
			lpw.y  = local_pos.x;
			lpw.z  = local_pos.y;
			lpw.vY = 0;
			lpw.vZ = 0;

			CUDA_CALL(cudaMemcpy(&lpww, &d_particle_weights[global_winner_index], sizeof(float), cudaMemcpyDeviceToHost));
		...
		if (num_transfer > 0)
		{
			exchange_ring<<< num_blocks, num_transfer>>>(d_particle_data_tmp, d_particle_weights, num_particles, num_blocks,
				thrust::raw_pointer_cast(&d_block_winners_weight[0]),
				thrust::raw_pointer_cast(&d_block_winners_index[0]));
//exchange_2dtorus<<< num_blocks, num_transfer>>>(d_particle_data_tmp, d_particle_weights, num_particles, num_blocks, num_transfer, griddimx, griddimy);
			e = cudaGetLastError();
			if (e != cudaSuccess) {
...
#endif /* MTGP */
	CUDA_CALL(cudaFree(d_particle_data.angles1));
//	CUDA_CALL(cudaFree(d_particle_data.angles2));
	CUDA_CALL(cudaFree(d_particle_data.pos));

	CUDA_CALL(cudaFree(d_particle_data_tmp.angles1));
//	CUDA_CALL(cudaFree(d_particle_data_tmp.angles2));
	CUDA_CALL(cudaFree(d_particle_data_tmp.pos));
