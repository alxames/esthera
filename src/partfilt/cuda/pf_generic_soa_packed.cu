...
#include <pf_model_soa_packed.cuh>
...
*/
	...
	state_data d_particle_data;
	{
		...
		{
			//init_particle_data[i] = init_state;
			const float a = 0.75 * M_PI;
			for (int j=0; j < NUM_ANGLES; ++j)
			{
				if      (j==0){init_particle_data.angles1[i].x = a;}
				else if (j==1){init_particle_data.angles1[i].y = a;}
				else if (j==2){init_particle_data.angles1[i].z = a;}
				else if (j==3){init_particle_data.angles1[i].w = a;}
				else if (j==4){init_particle_data.angles2[i]   = a;}
			}
			init_particle_data.pos[i].x = 7;
			init_particle_data.pos[i].y = 0;
