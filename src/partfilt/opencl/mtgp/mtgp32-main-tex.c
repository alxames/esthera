...
//	ccudaMalloc((void**)&d_data, sizeof(uint32_t) * num_data);
	...
	clerr = clSetKernelArg(mtgp32_uint32_kernel, 4, sizeof(cl_mem), &d_tex_param); CLERR;
	clerr = clSetKernelArg(mtgp32_uint32_kernel, 5, sizeof(cl_mem), &d_tex_temper); CLERR;
	clerr = clSetKernelArg(mtgp32_uint32_kernel, 6, sizeof(cl_int), &size); CLERR;
...
*/
	...
	d_tex_param = clCreateImage2D(context, CL_MEM_READ_ONLY, &image_format, block_num, TBL_SIZE, 0, NULL, &clerr); CLERR;
	d_tex_temper = clCreateImage2D(context, CL_MEM_READ_ONLY, &image_format, block_num, TBL_SIZE, 0, NULL, &clerr); CLERR;
	d_tex_single_temper = clCreateImage2D(context, CL_MEM_READ_ONLY, &image_format, block_num, TBL_SIZE, 0, NULL, &clerr); CLERR;
//	ccudaMalloc((void**)&d_status, sizeof(mtgp32_kernel_status_t) * block_num);
	r = num_data % num_unit;
	if (r != 0) {
		num_data = num_data + num_unit - r;
	}
	make_constant(d_params, MTGPDC_PARAM_TABLE, block_num);
	make_texture(d_tex_param, d_tex_temper, d_tex_single_temper, MTGPDC_PARAM_TABLE, block_num);
	make_kernel_data32(d_status, MTGPDC_PARAM_TABLE, block_num);
	make_uint32_random(d_status, d_params, d_tex_param, d_tex_temper, num_data, block_num);
	make_single_random(d_status, d_params, d_tex_param, d_tex_single_temper, num_data, block_num);
