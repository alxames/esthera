 ...
 */
/*__device__*/
uint32_t para_rec(uint32_t X1, uint32_t X2, uint32_t Y, int bid, __constant struct mtgp32_param_t *params, __read_only image2d_t tex_param) {
 ...
 */
/*__device__*/
uint32_t temper(uint32_t V, uint32_t T, int bid, __constant struct mtgp32_param_t *params, __read_only image2d_t tex_temper) {
	uint32_t MAT;
	sampler_t sampler = CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_NONE | CLK_FILTER_NEAREST;
 ...
 */
/*__device__*/
float temper_single(uint32_t V, uint32_t T, int bid, __constant struct mtgp32_param_t *params, __read_only image2d_t tex_single_temper) {
	uint32_t MAT;
	uint32_t r;
	sampler_t sampler = CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_NONE | CLK_FILTER_NEAREST;
...
__kernel void mtgp32_uint32_kernel(
		...
		__read_only image2d_t tex_param,
		__read_only image2d_t tex_temper,
		int size) {
	...
	for (int i = 0; i < size; i += LARGE_SIZE) {
		...
		r = para_rec(status[LARGE_SIZE - N + tid],
				...
				tex_param);
		status[tid] = r;
		o = temper(r, status[LARGE_SIZE - N + tid + pos - 1], bid, params, tex_temper);
		...
		o = temper(r, status[tid + pos - 1 + 2 * THREAD_NUM - N], bid, params, tex_temper);
...
__kernel void mtgp32_single_kernel(
		...
		__constant struct mtgp32_param_t* params,
		__read_only image2d_t tex_param,
		__read_only image2d_t tex_single_temper,
		int size)
{
	...
	for (int i = 0; i < size; i += LARGE_SIZE) {
		r = para_rec(status[LARGE_SIZE - N + tid],
				...
				tex_param);
		status[tid] = r;
		o = temper_single(r, status[LARGE_SIZE - N + tid + pos - 1], bid, params, tex_single_temper);
		d_data[size * bid + i + tid] = o;
		barrier(CLK_LOCAL_MEM_FENCE);
		r = para_rec(status[(4 * THREAD_NUM - N + tid) % LARGE_SIZE],
				status[(4 * THREAD_NUM - N + tid + 1) % LARGE_SIZE],
